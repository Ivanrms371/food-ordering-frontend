...
export const NavigationMenu = ({
  ...
  useEffect(() => {
    ...
    if (isMenuOpen) {
      setIsVisible(true);
      timeout = setTimeout(() => setAnimation(true), 50);
  ...
  return (
    <>
        ...
        <div className=" bg-gray-900/60 rounded-3xl p-8">
          <ul className="flex flex-col gap-2">
            {navLinks
              .filter(
                (link) =>
                  link.requiredAuth === undefined ||
                  link.requiredAuth === isisAuthenticated
              )
              .map((link) => {
        ...
        <div className="bg-gray-900/60 rounded-3xl p-8 px-12">
          <div className="flex gap-4 items-center justify-between">
            ...
            <div className="flex gap-2">
              <button
                type="button"
                className="rounded-full bg-gray-950 p-2 hover:bg-gray-800"
              >
                <svg
                  role="img"
                  viewBox="0 0 24 24"
      ...
      <div
        onClick={handleClose}
        className={twMerge(
          "fixed inset-0 bg-black/0 backdrop-blur-none z-20 transition-all duration-1000 cursor-pointer",
          isMenuOpen && "bg-black/40 backdrop-blur-sm "
        )}
      ></div>
    </>
  );
